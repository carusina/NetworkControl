...
3. **Play**: 클라이언트가 `play`를 보내면 그 세션의 `SessionState`가 `Playing`이 됨. Playing인 엔티티만 물리 시뮬레이션이 돌고 브로드캐스트를 주고받는다.
   ...
   - **스트래글러 패킷 방어**: `UdpReceiver`는 로컬에서 `Play`/`Pause`/`Stop`에 맞춰 `isActive_` 플래그를 켜고 끈다. Pause나 Stop을 보낸 뒤에도, 서버가 그 명령을 처리하기 전에 이미 날아오고 있던 `EntityState`가 뒤늦게 도착할 수 있는데, 그 시점엔 `isActive_`가 꺼져 있어서 `ReceiveWorker`가 파싱만 하고 `MetricsCollector`/`EntityWorld`에는 반영하지 않고 버린다 — 안 그러면 방금 리셋한 통계를 그 스트래글러 패킷이 다시 "첫 패킷"으로 오인해서 경과 시간이 다시 흐르거나, `SequenceId` 기준선이 어긋나 다음 세션의 진짜 패킷이 오히려 무시되는 문제가 생긴다.
...
미니맵 축은 표준 탑다운 지도 그대로 `(X, -Y)`다 — world `+X`는 오른쪽 그대로, world `+Y`는 화면 좌표(아래로 갈수록 커짐)에 맞추려고 Y만 반전. 인셋 좌하단에 "고정(회전 없음)"이라는 안내 텍스트를 같이 둬서, 메인 레이더(heading-up, 매 틱 회전)와는 기준이 다른 화면이라는 걸 명시했다(왜 표준 축으로 정리했는지는 아래 삽질 기록 5번 참고).
...
**5) 미니맵 축을 메인 레이더의 회전 결과에 맞춰 끼워 맞췄다가, 표준 축으로 되돌림** — 원래 미니맵 축은 `(-Y, -X)`였다. 이건 메인 레이더의 heading-up 회전식(`delta = 90° - cameraHeading`)이 `cameraHeading = 0`일 때 우연히 `canvasX ← -worldY`, `canvasY ← -worldX`가 되는 걸(90도 회전의 수학적 귀결일 뿐, 의도한 설계는 아니었음) 그대로 베껴서, 시작 시점에 두 화면의 "위"가 맞아 보이게 만든 값이었다. 문제는 미니맵이 "회전도 카메라 추적도 없는 절대 좌표 뷰"라고 스스로 정의해놓고, 정작 축은 메인 레이더의 회전 결과에 종속돼 있었다는 것 — 세계의 어떤 고정된 방향과도 원칙적으로 안 맞는 값이라 나중에 실제로 헷갈린다는 지적을 받았다. **고친 방법**: 미니맵 축을 표준 탑다운 지도 축인 `(X, -Y)`로 되돌리고(world `+X` = 오른쪽, world `+Y`를 화면 좌표에 맞춰 반전한 게 전부), 대신 인셋에 "고정(회전 없음)" 텍스트를 추가해서 메인 레이더와 기준이 다른 화면이라는 걸 명시적으로 안내함.
